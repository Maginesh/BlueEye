...
const AvgBattery = ({ data }) => {
  ...
  useEffect(() => {
    const telemetryData = data.reduce((acc, row) => {
      ...
      acc[row.telemetry_uid].sum += row.battery;
    ...
    const averagedData = Object.keys(telemetryData).map(key => ({
      telemetry_uid: key,
      avgBatteryLevel: telemetryData[key].sum / telemetryData[key].count,
    }));
  ...
  const options = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: 'Average Battery Level by Telemetry ID',
      },
    },
    scales: {
      x: {
