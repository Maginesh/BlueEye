...
import CsvReader from './GraphLmao/CsvReader';
import axios from 'axios';


export default function BulkDs() {
...
//   },[])
  ...
  const onSubmit = async () => {
      ...
      try {
        const response = await axios.post('http://127.0.0.1:5000/processBulkDs', formData, {
            headers: {
        ...
        const file = new File([response.data], 'processed_data.csv', {
            type: 'text/csv',
        });
        setExcelFile1(file); 
        setLoading(false);
        setActive(false);
        alert('CSV file processed successfully and stored in state!');
    } catch (error) {
        console.error('Error:', error);
        alert('File upload or processing failed. Please try again.');
        setLoading(false);
    }
